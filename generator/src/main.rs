...
fn write_file(file: &str, contents: &str) {
    ...
    let mut full_contents = disclaimer.as_bytes().to_owned();
    full_contents.extend(contents.as_bytes());
    std::fs::write(dst_path(file), &full_contents).unwrap();
}
...
fn dst_path(file: &str) -> PathBuf {
    let workspace_dir = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        .parent()
        .unwrap();
    let mut types_path = workspace_dir.to_owned();
    types_path.push("src");
    types_path.push(file);
    types_path
}
